...
test("database campaign continues after an ordinary website failure", async () => {
  ...
  const browserSummary = JSON.parse(
    await readFile(join(summary.runDirectory!, "browser-stage-summary.json"), "utf8"),
  ) as { entered: number; failures: number; notEntered: number; reconciliation: Record<string, boolean> };
  assert.equal(browserSummary.entered, 0);
  assert.equal(browserSummary.failures, 0);
  assert.equal(browserSummary.notEntered, 2);
  assert.equal(Object.values(browserSummary.reconciliation).every(Boolean), true);
  assert.match(
    await readFile(join(summary.runDirectory!, "browser-stage-failures.csv"), "utf8"),
    /^"site_id","website_url","category"/,
  );
});
