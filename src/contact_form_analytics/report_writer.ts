...
const browserStageFiles = (sites: SiteClassification[]): OutputFiles => {
  ...
  const rows = sites.map((site) => {
    const explicit = site.browserStage;
    const inferredFailure = site.terminalStage === "browser" && site.runState === "stopped";
    const inferredEntered = ["advanced", "qualified_not_executed", "completed", "stopped"].includes(
    ...
    return {
      ...
      mainDocumentReceived: explicit?.mainDocumentReceived,
      mainDocumentStatus: explicit?.mainDocumentStatus,
      meaningfulContent: explicit?.meaningfulContent,
  ...
  const loadedAfterTimeout = rows.filter((row) => row.outcome === "LOADED_AFTER_TIMEOUT").length;
  const categoryCounts = Object.fromEntries(
    categories.map((category) => [category, ledger.filter((row) => row.category === category).length]),
  ) as Record<(typeof categories)[number], number>;
  const categoryPercentagesOfFailures = Object.fromEntries(
    categories.map((category) => [
      category,
      ledger.length === 0 ? 0 : Number(((categoryCounts[category] / ledger.length) * 100).toFixed(2)),
    ]),
  ) as Record<(typeof categories)[number], number>;
  ...
  const subcategoryCounts = Object.fromEntries(
    [...new Set(ledger.map((row) => row.subcategory || "unknown_browser_failure"))]
      .sort()
      .map((subcategory) => [
        subcategory,
  ...
  const summary = {
    ...
    loaded,
    loadedAfterTimeout,
    failures: ledger.length,
    notEntered: sites.length - entered,
    categoryCounts,
    ...
    preBrowserExclusions: rows
      ...
      .map((row) => ({ websiteUrl: row.websiteUrl, reason: row.reason })),
    reconciliation: {
      entrantsEqualLoadedPlusFailures: entered === loaded + loadedAfterTimeout + ledger.length,
      failuresEqualCategorySum:
        ledger.length === Object.values(categoryCounts).reduce((sum, value) => sum + value, 0),
      failuresEqualLedgerRows: ledger.length === rows.filter((row) => row.outcome === "FAILED").length,
      ledgerSiteIdsUnique: new Set(ledger.map((row) => row.siteId)).size === ledger.length,
    },
  };
  const text = [
    "BROWSER-STAGE ANALYTICS",
    "=======================",
...
const outputFiles = async (result: OutreachAnalyticsResult): Promise<OutputFiles> => {
  ...
  return {
    ...
    ...prefixedFiles(path.join("stages", "browser"), browserStageFiles(forms.sites)),
