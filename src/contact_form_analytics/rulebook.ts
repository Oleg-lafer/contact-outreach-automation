...
const isBrowserTerminal = (site: SiteEvidence): boolean =>
  site.browserStage?.outcome === "FAILED" ||
  site.failureKind === "navigation.failed" ||
  (assessmentIs(site, "site_inspection_blocked") && /page\.goto|net::err_/i.test(primaryText(site)));
...
const baseRules: Array<Omit<OrderedRule, "order">> = [
  ...
  {
    id: "ENV-PRE-BROWSER-RUNTIME-FAILURE",
    stage: "pre_browser",
    title: "Runtime failed before browser entry",
    attribution: "workflow_attributable",
    causeFamily: "execution_environment_issue",
    subcategory: "pre_browser_runtime_failure",
    evidenceTier: "structured_text",
    description: "The run failed before structured or legacy evidence proved browser-stage entry.",
    matches: (site) =>
      !site.browserStage &&
      site.failureKind === "runtime.error" &&
      assessmentIs(site, "site_inspection_blocked") &&
      !/page\.goto|net::err_/i.test(primaryText(site)),
  },
  {
  ...
  {
    id: "BRW-STRUCTURED-OUR-SYSTEM",
    stage: "browser",
    title: "Structured browser evidence attributes failure to our system",
    attribution: "workflow_attributable",
    causeFamily: "execution_environment_issue",
    ...
    decide: (site) => ({
      ...
      primaryCause: site.browserStage?.reason || "Our system browser-stage failure",
    }),
  },
  {
    id: "BRW-STRUCTURED-DESTINATION",
    stage: "browser",
    title: "Structured browser evidence attributes failure to destination",
    attribution: "non_workflow_attributable",
    ...
    decide: (site) => ({
      ...
      primaryCause: site.browserStage?.reason || "Destination browser-stage failure",
    }),
  },
  {
    id: "BRW-STRUCTURED-ACCESS-OURS",
    stage: "browser",
    title: "Access restriction attributable to our system",
    attribution: "workflow_attributable",
    causeFamily: "execution_environment_issue",
    subcategory: "access_restriction_our_system",
    evidenceTier: "debug_artifact",
  ...
  },
  {
    ...
    stage: "browser",
    title: "Access restriction with unknown responsibility",
    attribution: "indeterminate",
    causeFamily: "insufficient_evidence",
    subcategory: "access_restriction_responsibility_unknown",
    evidenceTier: "debug_artifact",
  ...
  },
  {
    ...
    stage: "browser",
    title: "Destination access restriction",
    attribution: "non_workflow_attributable",
    causeFamily: "external_site_or_service_issue",
    subcategory: "destination_access_restriction",
    evidenceTier: "debug_artifact",
  ...
  },
  {
    ...
    stage: "browser",
    title: "Browser failure responsibility undetermined",
    attribution: "indeterminate",
    causeFamily: "insufficient_evidence",
    subcategory: "browser_failure_responsibility_undetermined",
    evidenceTier: "debug_artifact",
    description: "Phase-aware evidence is insufficient to assign browser-stage responsibility.",
    matches: (site) => site.browserStage?.outcome === "FAILED" && site.browserStage.category === "UNDETERMINED",
    decide: (site) => ({
      subcategory: site.browserStage?.subcategory || "browser_failure_responsibility_undetermined",
      evidenceSummary: site.browserStage?.reason || "Structured browser-stage evidence remains undetermined.",
      primaryCause: site.browserStage?.reason || "Undetermined browser-stage failure",
    }),
  },
  {
    id: "BRW-DNS-FAILURE",
...
export const classifySite = (site: SiteEvidence): SiteClassification => {
  if (site.hasOnlyInput) {
    ...
    return {
      ...
      ...(site.browserStage ? { browserStage: site.browserStage } : {}),
    };
  }
