...
Run-mode responsibilities are divided between the public entry point, macro core, and forms orchestrator. The root `contact_outreach_orchestrator.ts` validates and propagates the selected mode and manages mode-specific output configuration and environment state. The macro `orchestrator/contact_outreach_core_(Integration).ts` creates and finalizes the additional diagnostic context before browser launch when `deep-debug` is selected. The form-specific `contact_channels/forms/forms_orchestrator.ts` executes discovery, population, recovery, and submission and records form-specific diagnostics into that context. When changing run-mode names, contracts, defaults, or behavior, inspect and update all three files together.
