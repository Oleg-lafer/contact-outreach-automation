...
export async function run_database_campaign(
  ...
  const browser_outcomes: ContactOutreachOutcome[] = [];
  const pre_browser_exclusions: Array<{ websiteUrl: string; reason: string }> = [];

  try {
    for (const [candidate_index, candidate] of candidates.entries()) {
      const claim = await repository.claimWebsite(campaign, candidate.websiteId, mode);
      if (claim.action === "unavailable") {
        summary.unavailable++;
        pre_browser_exclusions.push({
          websiteUrl: candidate.websiteUrl,
          reason: "The database claim was unavailable before browser execution.",
        });
        continue;
      }
      ...
      if (claim.action === "skip") browser_outcomes.push(outcome);
    }
  } finally {
    await write_browser_stage_run_summary_files(
      runDirectory,
      create_browser_stage_run_summary(
        browser_outcomes,
        dependencies.now(),
        pre_browser_exclusions,
      ),
    ).catch((error: unknown) => {
      console.warn(
        `Could not write browser-stage campaign diagnostics: ${error instanceof Error ? error.message : String(error)}`,
      );
    });
  }
